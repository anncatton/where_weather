...
# File.open('./weather_data/station_name_map.rb') { |file| file.write(STATION_NAME_MAP) }
...
def read_and_write_uri(uri, filename)
		...
		# here pretty name is a hash containing city, region and country
...
weather_files.each do |file|
	...
	open(file) do |f|
		...
		stations = response.map do |ea|
			Station.from_hash(ea)
		end

		stations.each { |ea| ea.print }
	end
	# open(file) do |f|
	# 	json_file = f.read
	# 	parsed_file = JSON.parse(json_file)
...
# # if there's no "region" (state), just put nothing.
# unless 
