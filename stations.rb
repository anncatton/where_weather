...
# going to need to test this out with the format that time is in. do i need to change the format that's given in the server response?
	...
	def print_matches_in(stations)
		...
		unless matching.empty?
		  place_names = matching.map do |ea|
		  	if ea.pretty_name
			  	ea.pretty_name['city'] + ", " + ea.pretty_name['country']
			  else
			  	puts ea.name + "pretty_name missing"
			  end
			end
...
# i think you're eventually going to have to go through the whole csv file and make sure the data is accurate. i'd hate to have inaccurate location data
