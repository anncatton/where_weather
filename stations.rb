...
def build_station_name_map
	...
	station_hash = station_names.map do |ea|
		ea.to_hash
	end

	stations_by_icao_id = {}
	station_hash.map do |ea|
		unless ea['icao_xref'].nil?
			stations_by_icao_id[ea['icao_xref']] = ea['city']
		end
	end
...
def read_and_write_uri(uri, filename)
		open(uri) do |io|
		json_string = io.read
		File.open(filename, 'w') { |file| file.write(json_string) }
	end
end
...
# 		# so here i think andy wants me to parse the json before i even write it to the individual file?
# 		# and put in the pretty name from the csv file too. would i create a new key in the hash to hold
# 		# the pretty name?
# 		# parse json
# 		# add pretty name
# 		# writing
